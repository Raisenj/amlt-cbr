...
"""This file implements the interactive shell to use the CBR tool"""
...
class CBR(Console):
    ...
    def do_loadCasesFromCsv(self, args):
        """Loads the cases from the specified csv file. Example:
            loadCasesFromCsv <local path to filename>"""
    ...
    def do_setRetrieveParameter(self, args):
        """Set the K parameter for the retrieve phase. How to use:
            setRetrieveParameter <K>"""
        try:
            params = args.split()
            if len(params) != 1:
                raise Exception('wrong number of arguments')
    ...
    def do_setMemoryModel(self, args):
        """Set the memory model: flat or hierarchical. How to use:
            setMemoryModel <flat|hierarchical>"""
        try:
            params = args.split()
            if len(params) != 1:
                raise Exception('wrong number of arguments')
    ...
    def do_executeCBR(self, args):
        """Runs the CBR with the loaded case"""
        try:
    ...
    def do_testDataSet(self,args):
        """ Executes a test over the loaded dataset"""
